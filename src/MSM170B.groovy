...
class MSM170B extends MSOHook{
    ...
    def integrationMaximo(GenericMsoRecord request, GenericMsoRecord response, String hostUrl){
        ...
        String stockCode = request.getField("STOCK_CODE2I").getValue()
...
//      String stockDesc = StringEscapeUtils.escapeXml(request.getField("ITEM_NAMEA2I").getValue().trim())
        String unitOfIssue = StringEscapeUtils.escapeXml(request.getField("UNIT_OF_ISSUE2I").getValue().trim())
        ...
        Constraint constStockCode = MSF170Key.stockCode.equalTo(stockCode)

        Query queryMSF170 = new QueryImpl(MSF170Rec.class).and(constDistrict).and(constStockCode)
        MSF170Rec msf170Rec = tools.edoi.firstRow(queryMSF170)

        log.info("query 170 result: ${msf170Rec}")

        if (msf170Rec){
            String warehouseId = msf170Rec.getHomeWhouse() ? msf170Rec.getHomeWhouse().trim() : ""
            String primaryCateg = msf170Rec.getPrimaryCat() ? msf170Rec.getPrimaryCat().trim() : ""

            log.info("warehouseId: $warehouseId")
            log.info("primaryCateg: $primaryCateg")
